    ...
    export const approveCharacter = async (req, res) => {
        try {
            ...
            // 🟢 AUTO-APPROVE ASSETS: Also approve all side profile shots belonging to this character
            const projectAssets = await prisma.asset.findMany({ 
                where: { projectId: id, type: 'CHARACTER' } 
            });
            
            const charAssetsToApprove = projectAssets.filter(a => {
                try {
                    const meta = JSON.parse(a.metadata || '{}');
                    return meta.characterId === charId;
                } catch (e) { return false; }
            ...
            if (charAssetsToApprove.length > 0) {
                await prisma.asset.updateMany({
                    where: { id: { in: charAssetsToApprove.map(a => a.id) } },
                    data: { state: 'APPROVED' }
                });
                logger.info({ charId, assetCount: charAssetsToApprove.length }, 'Auto-approved character side profile assets');
            }
    ...
    export const approveAllCharacters = async (req, res) => {
        try {
            ...
            // 🟢 AUTO-APPROVE ALL CHARACTER ASSETS
            await prisma.asset.updateMany({
                where: { projectId: id, type: 'CHARACTER' },
                data: { state: 'APPROVED' },
            });
            logger.info({ projectId: id }, 'Auto-approved all character assets for project');
     ...
     * Force-stops a running generation by setting the project state to CANCELLED.
     * The worker checks this state before every single API call to save credits.
     */
    export const cancelProject = async (req, res) => {
        try {
            ...
            logger.info({ projectId: id }, 'User cancelled project generation');
            res.json({ message: 'Project generation cancelled. Workers will abort shortly.', state: 'CANCELLED' });
        } catch (error) {
            logger.error({ err: error }, 'Cancel project failed');
